...
계약을 하한으로만 쓰고 위쪽 여유를 쓰게 하면 이 손해는 사라지지만, 빈 자리를 먼저 온 요청이 가져가므로 **도착 순서가 등급을 이긴다.** 이질 워크로드 기준 구분 지표가 25.4에서 1.24로 무너지며, 이는 C1(2.06)보다도 낮다. 상향 여유를 25%로 제한하면 2.23으로 회복되는 대신 활용률 이득이 줄어든다.
