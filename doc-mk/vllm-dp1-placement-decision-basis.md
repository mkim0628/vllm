...
## 1.2 대응 — 차세대 메모리의 등장과 이기종 혼재

이 벽을 한 종류의 메모리로 넘을 수 없으므로, 용량·대역폭·비용의 조합이 서로 다른 메모리 솔루션이 등장하고 있다. 그 결과 시스템에는 성격이 자릿수 단위로 다른 메모리가 **혼재**한다.

| tier | 대역폭 | 용량(예시) | 지연 | 이동비용 |
|---|---:|---:|---:|---:|
...
따라서 이기종 메모리가 혼재하는 환경에서는 런타임이 **각 메모리의 성격에 맞게 데이터를 배치**해야 한다.
...
| 대가 | 내용 |
|---|---|
| **변경 파급** | 배치 경로 T=6개 + 이동 경로 T×(T−1)=30쌍 → 스케줄러 안의 분기 **36개**. 메모리 구성이 바뀔 때마다 스케줄러를 고쳐야 한다 |
...
```text
 Scheduler ──── "n 블록" ────► Placer ────► HBM │ cHBM │ DRAM │ CXL │ HBF │ SSD
                 (요청은 익명)      ▲
                                  │
                            Tier 상태표          ← 자원이 결정한다
                        용량 · 대역폭 · 이동비용
...
배경 1.3의 비어 있던 자리에 **Placer**를 놓고, 거기에 **Tier 상태표**를 물린 형태다. 스케줄러는 여전히 tier를 모른다.
...
같은 자리에 같은 **Placer**를 놓되, 물리는 것이 **객체 등급/비용 모델**이다. 스케줄러는 여전히 tier를 모른다.
...
>
> 다른 것은 둘뿐이다.
> 1. **화살표 라벨** — `"n 블록"` vs `"n 블록 + 요청"`. 익명성 차이가 여기서 드러난다.
