...
|---|---|---|---|---|
| **문제 한 문장** — 위치를 정할 기준을 담을 자리가 없어 tier 선택이 호출 지점 조건문으로 흩어진다 | 참 | 자리 = `TierStateTable`. 조건문 소멸 | 자리 = `Class-Tier Contract`. 조건문 소멸 | **둘 다 해결** |
| QA1 영향: 구분 가능한 객체 등급 | 1종 (도착 순서로 상위 tier 점유) | **1종 — 미해소** | 8종 — 해소 | C1 부분 / C2 해결 |
| QA2 영향: 결정 지표 수와 갱신 정의 | 1개(`get_num_free_blocks`), 갱신 주기 미정의 | 24개, 스텝당 갱신 정의됨 | 24개 + 특성 3개(추정), 갱신 정의됨 | 둘 다 해결 (비용 차이) |
| QA3 영향: 오배치 교정 경로 | 없음 | 다음 할당에 자동 반영 — 해소 | **없음 (정적 고착) — 미해소** | C1 해결 / C2 미해소 |
| QA4 영향: 결정 근거 기록 | 없음, 사후 추정 필요 | 24개 스냅샷 필요 — **부분 해소** | 클래스 라벨 1개 — 해소 | C1 부분 / C2 해결 |
| 제약: `allocate_slots` 호출 규약 유지 | — | 준수 (인자 불변) | 준수 (`request`는 이미 인자에 있다) | 위반 없음 |
...
- **그러나 어느 후보도 QA 4개를 모두 해결하지 못한다.** C1은 QA1을 미해소로, C2는 QA3을 미해소로 남긴다.

> 9 : 9 동점의 실체가 여기서 드러난다. 두 후보는 같은 문제를 풀고 **서로 다른 잔여를 남긴다.**
...
> 근거는 1장의 제약 두 가지다 — 배치 결정이 스케줄 스텝 임계 경로에 있고, 요청 인터페이스를 건드리지 않아야 한다.
> 그리고 C2가 QA1에서 얻는 이득은 **출력 길이·재사용 횟수 예측 정확도에 전적으로 의존하는데 그 값이 아직 측정되지 않았다**(부록 D.4).
> 미측정 상태에서 C2를 고르면 QA2 비용과 QA3 고착은 확실히 지불하고 QA1 이득은 불확실하다.
>
> 따라서 **Candidate 1 (Tier-Indexed)** 을 최종 구조로 선택한다.
>
> 이 선택으로 포기하는 것은 **QA1(구분 능력)과 QA4(재현성)** 이며, 아래 보강으로 다룬다.
...
| QA4 재현성 | 배치 결정 시 tier score 스냅샷을 결정 로그로 남긴다(24개 값 → 요약 스칼라 3~4개) | 재현은 불가하나 **사후 설명은 가능**해진다 |

## 혼합 판정 — 이것은 혼합이 아니라 "C1 + 보강"이다

컨텍스트 길이를 score에 넣으면 객체 정보를 쓰는 셈이지만, **단일 진실 원천은 여전히 `TierStateTable` 하나**다.
객체 정보는 정책의 인덱스가 아니라 score의 한 항(가중치)으로만 들어가며, 경합 시 불변식은 그대로 "자원 제약 우선"이다.
...
| **하류** | **DP2** (Compute-Capable Memory Abstraction) | 배치가 곧 실행 가능성을 결정한다. tier 선택 기준에 capability를 넣을 수 있는지는 DP2가 capability를 어디에 두느냐에 달려 있다 — DP2의 C1(Capability-in-Memory)이면 tier의 정적 속성으로 표현 가능하고, C2(Capability-in-Binding)면 tier 상태만으로는 표현할 수 없다 |
| **하류** | **DP3** (신규 — 재배치 트리거) | DP1의 인덱스 축이 DP3의 신호원을 제약한다 (R1) |
| **하류** | **DP4** (신규 — 요청 특성 예측) | DP4의 예측 정확도가 DP1의 반전 조건을 공급한다 (R2) |
...
| G. 문제 해결 검증 | 통과 | 배경의 문제·QA 영향·제약을 그대로 검증 항목으로 환원, As-Is → To-Be 표기, 제약 위반 없음, 두 후보 모두 핵심 문제 해결 |
| H. 최종 선택 | 통과 | 별점 합계가 아닌 QA 가중치로 선택(제약 2건에서 도출), 포기한 축과 완화책 명시, PoC 3건에 판정 임계값 고정, 반전 조건 3건 |
| I. 잔여 인계 | 통과 | 잔여 3건 각각에 "왜 이 DP에서 못 푸는가" 답변, 인계 DP 지목과 결합 방향, DP 결합 맵 수록. 최종안은 진짜 혼합이 아니라 "C1 + 보강"으로 판정 |
...
세 번째는 **총량 활용 ↔ 구분 능력**이며, 프로토타입 실측에서 드러났다(`doc-mk/prototype`). C2에서 tier 상태는 기본적으로 **아래로만** 작용한다 — 계약이 지시한 tier가 차면 내려가지만 위가 비어도 올라가지 않는다. 그래서 한 등급이 워크로드를 지배하면 상위 tier가 비어 있는데도 쓰지 않는다(균질 워크로드에서 요구 가중 대역폭 달성률 0.02 대 C1의 1.00).

계약을 하한으로만 쓰고 위쪽 여유를 쓰게 하면 이 손해는 사라진다. 그러나 구분 지표가 이질 워크로드 기준 25.4에서 1.24로 무너지며, 이는 C1(2.06)보다도 낮다. 상향 여유를 25%로 제한하면 2.23으로 회복되는 대신 활용률 이득이 줄어든다.

C1보다도 낮아지는 이유는 두 가지가 겹치기 때문이다. 첫째, C1의 점수 함수에는 점유율 페널티가 있어 상위 tier가 찰수록 밀려나지만, 상향 규칙은 "자리가 있으면 위로"라는 이진 판정이라 페널티가 없다. 둘째, **크기 비대칭 편향**이다 — 짧은 요청은 32블록, long-context 요청은 2048블록이므로, 이진 판정 아래에서는 큰 요청이 못 들어가는 틈에도 작은 요청이 들어간다. 상위 tier가 작은 객체로 잠식되고 정작 대역폭이 필요한 큰 객체가 밀려난다. 실측에서 짧은 요청 48개의 평균 tier 대역폭이 C1에서는 1475(세 tier에 분산)인데 상향 무제한에서는 2400(상위 두 tier에 집중)이 된다.

> C1의 점유율 페널티는 herding 완화 장치일 뿐 아니라 **크기 편향을 완화하는 장치**이기도 하다.
...
따라서 QA1의 ★★★는 **상향을 닫았을 때의 값**이다. 그리고 이 손잡이를 제대로 쓰려면 상위 tier의 일부를 고등급 전용으로 유보해야 하는데, 그것은 C1이 herding을 막기 위해 쓰는 예약 카운터와 같은 메커니즘이다 — **두 후보 모두 '유보'라는 공통 요소를 필요로 한다.**
