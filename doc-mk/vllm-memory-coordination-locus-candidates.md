...
- **후보 A(중앙집중) × DP-1 후보 1(범용성)**: 잘 맞습니다. 오케스트레이터가 모든
  티어를 하나의 관점에서 봐야 하므로, 티어 인터페이스가 범용적일수록
  오케스트레이터 구현이 단순해집니다. 반대로 후보 A와 DP-1 후보 2(특화)를
  같이 쓰면, 오케스트레이터가 결국 모든 특화 인터페이스를 다 이해해야 해서
  DP-1에서 피하려던 "중앙이 모든 하드웨어 특성을 알아야 하는 문제"가 조정
  로직 쪽에서 재발합니다.
- **후보 B(분산 자율) × DP-1 후보 2(특화)**: 잘 맞습니다. 각 티어가 자기
  이웃과 협상할 때, 자기 하드웨어 고유 특성(예: CXL의 pooling 정보)을 협상
  프로토콜에 반영하기 쉽습니다. 반대로 후보 B와 DP-1 후보 1(범용)을 같이
  쓰면, 협상에 쓸 수 있는 정보가 범용 필드로 제한되어 분산 협상의 이점(하드웨어
  고유 판단)을 살리기 어려워집니다.

즉 **"후보 A + DP-1 후보 1"과 "후보 B + DP-1 후보 2"가 자연스러운 짝**이고,
반대 조합(후보 A + DP-1 후보 2, 후보 B + DP-1 후보 1)은 각 축의 장점을 서로
상쇄시키는 조합입니다.
