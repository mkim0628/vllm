...
- `MemoryTier`(`CXLTier`/`HBFTier`)는 항상 수동적 자원 객체입니다 — 스스로
  ...
  `TierDataMover`의 `copy_out`/`copy_in`(STAGED) 또는 `get_dma_handle`/
  `receive_dma`(DIRECT) 호출에 응답할 뿐입니다.
- 실제 물리적 전송은 `TierDataMover`(두 후보 공통 컴포넌트)가 수행하며, 두
  티어가 같은 cache-coherent fabric에 있는지에 따라 DIRECT(host를 거치지
  않고 두 디바이스가 직접 DMA)와 STAGED(host DRAM을 중간 버퍼로 경유) 중
  하나를 씁니다 — 자세한 배경은
  `vllm-memory-coordination-locus-candidates.md` §1.
...
7. **`TierDataMover`가 양쪽 `MemoryTier`의 `capabilities()`를 비교해서
   DIRECT/STAGED 중 하나를 결정**합니다 — 4단계의 견적과 같은 판단 로직을
   그대로 실행에 씁니다. `CXLTier`/`HBFTier` 어느 쪽도 이 판단에 관여하지
   않습니다.
8. **(DIRECT 분기) 두 티어가 같은 cache-coherent fabric에 있으면**,
   `TierDataMover`가 `CXLTier.get_dma_handle(block_ids)`를 불러 실제
   바이트가 아니라 물리 주소 등을 담은 가벼운 `MemoryHandle`만 받고,
   `HBFTier.receive_dma(handle, block_ids)`로 넘깁니다. 이후 두 디바이스가
   직접 DMA로 복사하고, **바이트는 `TierDataMover`가 도는 host DRAM에
   전혀 들어오지 않습니다.**
9. **(STAGED 분기) 두 티어가 직접 연결되어 있지 않으면**,
   `TierDataMover`가 `CXLTier.copy_out(block_ids)`로 데이터를 꺼냅니다 —
   이 반환값은 호출자, 즉 `TierDataMover`가 도는 **host DRAM에 실제로
   만들어집니다.** 그 뒤 `HBFTier.copy_in(block_ids, bytes)`로 써넣습니다.
   `src → host DRAM → dst`의 2단계이고, 어느 분기에서든 `CXLTier`와
   `HBFTier`는 "자기 자신의 메모리에서 내보내기/받기"라는 원시 동작만
   수행할 뿐, 서로를 직접 호출하지 않습니다.
10. **`TierDataMover`가 `GlobalMemoryOrchestrator`에 이관 완료를 보고**합니다.
11. **`GlobalMemoryOrchestrator`가 전역 상태 테이블을 갱신**합니다
...
- 새로 만들어야 할 것: `GlobalMemoryOrchestrator`(기존 `TierPlacementPolicy`의
  ...
  `estimate_transfer_cost` 견적 조회 + `transfer` 실행, DIRECT/STAGED 판단
  로직과 `MemoryHandle` 타입 포함).
...
- 이 시퀀스는 별(star) 토폴로지입니다 — `CXLTier`와 `HBFTier`가 서로 직접
  ...
  전송(7~9단계, DIRECT/STAGED 분기 포함)은 시나리오 10과 완전히 동일한
  방식으로 이뤄집니다 — 다른 건 오직 그 이전의 "누가, 얼마나 넓은 정보로
  결정하는가"입니다.
