...
**KV는 경로를 건너지 않는다.** 건너는 것은 Q, K_new/V_new, attn_out — 모두 **토큰 수 × hidden** 크기의 활성화 텐서다.

신규 토큰의 K/V도 같은 메모리에 append되므로 **Attention 전체가 한 곳에서 끝나고 softmax 재정규화가 필요 없다.**

### 4.2 경로를 건너는 바이트 — 정량

아래는 **가정 모델**(dense 70B급: hidden 8192, 계층 80, heads 64, head_dim 128, GQA kv_heads 8, fp16, context 32K)에서의 계산이다. 숫자 자체보다 **자릿수 차이**가 요점이다.

```text
KV 크기
  토큰·계층당 = 2(K,V) × 8 heads × 128 dim × 2B = 4 KiB
  토큰당 전체 = 4 KiB × 80 계층                  = 320 KiB
  32K context                                    = 10.0 GiB   ← 세션 하나

Decode 1 token, 계층당 경로 통과량
  Q          64 × 128 × 2B = 16 KiB
  K_new,V_new                =  4 KiB
  attn_out   64 × 128 × 2B = 16 KiB
  ─────────────────────────────────
  계층당                     ≈ 36 KiB
  × 80 계층                  ≈ 2.81 MiB   ← Decode 1 token 전체
```

**KV 10 GiB vs 활성화 2.81 MiB — 약 3,600배 차이**다. 경로가 좁을수록 이 구조가 값을 한다.

### 4.3 계층당 왕복 지연 — 이 구조의 비용

바이트는 작지만 **왕복 횟수가 계층 수만큼 늘어난다.**

```text
Decode 1 token당 경로 왕복 = 80회 (계층당 1회)

Custom HBM 노드 (fabric, 지연 1 us)
  전송      2.81 MiB / 0.9 TB/s  ≈   3 us
  왕복 지연 80 × 1 us            ≈  80 us
  ───────────────────────────────────────
  링크 오버헤드                  ≈  83 us / token

CXL-PNM (CPU 경유, 지연 250 ns × 2홉 ≈ 0.5 us 가정)
  전송      2.81 MiB / 64 GB/s   ≈  46 us
  왕복 지연 80 × 0.25 us         ≈  20 us
  ───────────────────────────────────────
  링크 오버헤드                  ≈  66 us / token
```

두 경우 모두 Decode step 시간(§4.4의 1.4~9.8 ms) 대비 **1% 미만**이다. 다만 **지연이 μs 급인 SSD-PIM에서는 성립하지 않는다** — 80회 × 60 μs = 4.8 ms가 되어 대화형 경로에서 무시할 수 없다. §3.2가 SSD-PIM을 오프로드 대상에서 제외한 이유 중 하나다.

배치 안의 여러 요청이 같은 계층을 함께 처리하면 왕복이 상쇄되므로 **이 비용은 배치 크기에 반비례**한다. 시뮬레이션에서 배치 크기를 Sweep 축에 넣어야 하는 이유다.

### 4.4 Decode Step 비용 — 경로별 비교

§3.4 Configuration과 §4.2 가정 모델(세션 KV 10 GiB)로 계산한 **Decode 1 step 비용**이다.

| 경로 | Step 시간 | 로컬 HBM 대비 | 비고 |
|---|---:|---:|---|
| **HBM 로컬** (Mode A) | **1.34 ms** | 1.00× | 기준. 용량 192 GiB |
| Custom HBM 노드, fabric read (Mode A) | 11.93 ms | **8.89×** | KV를 fabric으로 읽으면 이렇게 된다 |
| **Custom HBM 노드, Attention 오프로드 (Mode C)** | **1.43 ms** | **1.06×** | 링크 83 μs 포함. **용량 1 TiB** |
| CXL-PNM, Attention 오프로드 (Mode C) | 9.83 ms | 7.32× | 링크 66 μs 포함. 용량 512 GiB |
| HBF, GPU 직접 read (Mode A) | 10.74 ms | 8.00× | 연산 불가. 보관 계층 |

**가장 중요한 두 줄은 Custom HBM 노드의 두 행이다.**

> **같은 메모리에 대해 Mode A(11.93 ms)와 Mode C(1.43 ms)가 8.3배 차이난다.**
> fabric으로 KV를 읽으면 로컬 HBM보다 8.9배 느리지만, 노드 안에서 Attention을 수행하면
> **로컬 HBM과 대등한 처리량(1.06×)을 내면서 용량은 5배**가 된다.
> 이 노드에서 **Attention 오프로드는 선택이 아니라 사실상 전제**다.

CXL-PNM은 오프로드해도 로컬 HBM의 7.3배 느리다. **이 계층의 값어치는 속도가 아니라 용량당 비용**이며, 재접근이 드문 KV를 받는 자리다.

### 4.5 복원과 오프로드의 손익분기

재활성 이후를 두 방식으로 비교하면 손익분기가 **턴당 생성 토큰 수**로 나온다.

```text
방식 B. 복원 후 로컬 HBM에서 Attention
  재활성 1회      10 GiB / 외부 대역폭
  Decode step당   1.34 ms (로컬 HBM)

방식 C. Attention 오프로드
  재활성 1회      0
  Decode step당   §4.4의 Mode C 값
```

| 메모리 | 복원 비용 (1회) | Mode C step | 손익분기 N |
|---|---:|---:|---:|
| **CXL-PNM** | 167.8 ms | 9.83 ms | **≈ 20 tokens** |
| **Custom HBM 노드** | 11.9 ms | 1.43 ms | **≈ 143 tokens** |
| (참고) HBF 복원 | 10.7 ms | 오프로드 불가 | — |

> **CXL-PNM:** 턴당 생성이 20 토큰보다 짧으면 오프로드가 유리하다. Tool Call을 내는 턴의 출력은 대개 짧은 구조화 텍스트이므로 **Agent 워크로드는 유리한 쪽에 있는 경우가 많고**, 최종 답변을 길게 쓰는 턴은 복원이 유리하다.
>
> **Custom HBM 노드:** 손익분기가 143 토큰으로 훨씬 높다. 복원이 fabric을 타서 12 ms로 싸기 때문이다.
>
> **단, 이 손익분기만 보고 판단하면 안 된다.** 복원은 **로컬 HBM 10 GiB를 소비**한다. Custom HBM 노드를 둔 이유 자체가 HBM 용량을 아끼는 것이므로, "복원이 조금 더 빠르다"가 "복원하는 게 맞다"를 뜻하지 않는다. **속도와 용량을 함께 보는 지표가 §11.2의 M-P1(Goodput)이고, 용량 쪽 귀결을 보는 것이 M-P6이다.**

**이 계산이 기대는 가정을 명시한다.** §3.5의 가정값 두 개가 이 표를 직접 만든다.

- **Custom HBM 노드의 내부 대역폭 8.0 TB/s** — 이 값이 절반(4 TB/s)이면 Mode C step이 2.68 ms가 되어 로컬 HBM 대비 2배가 되고, "대등하다"는 결론이 무너진다.
- **연산형 메모리가 내부 대역폭을 온전히 쓴다는 가정** — 실제 근접 연산 유닛이 Attention 커널에서 내부 대역폭을 얼마나 달성하는지는 공개 수치가 없다. 달성률 50%면 위와 같은 효과다.

> **§11.6의 Sweep에 "연산형 메모리의 Attention 대역폭 달성률"과 "Custom HBM 노드 내부 대역폭"을 반드시 넣는다.** 고정값으로 두고 결론을 내면 안 된다.
...
| 조건 | 내용 |
|---|---|
| **원시 연산 지원** | `QK_GEMM`, `SOFTMAX`, `AV_GEMM`, `CAUSAL_MASK`를 모두 지원해야 한다. 하나라도 빠지면 Attention이 그곳에서 끝나지 않는다 — `ssd_pim`이 이 경우다 (§3.4) |
| **지연 예산** | 계층당 왕복 × 계층 수가 TPOT 예산 안에 들어와야 한다. Custom HBM 83 μs, CXL-PNM 66 μs는 통과하지만 SSD-PIM 4.8 ms는 아니다 (§4.3) |
| **용량** | 연산형 메모리가 모든 세션의 History를 담을 수 없다. **선택받은 일부만** 갈 수 있다 — 배치 정책이 필요한 이유를 강화한다 |
| **재사용 확률** | 세션이 돌아오지 않으면 연산형 메모리 자리를 낭비한 것이다. §5.1의 Reuse Probability 추정이 필요하다 |
...
| **Custom HBM 노드 내부 대역폭** | §3.5의 가정값(8.0 TB/s). §4.5가 밝혔듯 이 값이 절반이면 "오프로드가 로컬 HBM과 대등"이라는 결론이 무너진다 |
...
| **GPU Occupancy** | Attention을 메모리로 보낼 때 GPU 연산 유닛이 비는 이득. **이 항이 없으면 연산 가능 메모리 수치가 전부 하한**이다 |
