...
- **함의:** 활성 Decode를 여기서 돌리기엔 느리다. **대용량 보관 + 빠른 복원**이 이 계층의 역할이다 — 복원이 10.7 ms로 CXL-PNM(168 ms)의 1/16이다. 다만 **왕복이 반복되면 수명이 소모**되므로(§11.5 M-E1) 왕복 빈도가 높을 KV는 피해야 한다.
...
| **Custom HBM** | **0.9 TB/s** | **8.0 TB/s** | **8.9×** | **사실상 전제** — 안 하면 8.9배 손해 |
| **CXL-PNM** | **0.064 TB/s** | **1.1 TB/s** | **17.2×** | **가장 값이 큼** — 안 하면 17배 손해 |
...
§3.5의 가정값은 시뮬레이션 **이전의 예측**을 세우기 위한 것이다. 실제 수치가 들어와야 시뮬레이션 결과를 POC에 연결할 수 있다. 아래는 **무엇을 물어야 하고, 모르면 어느 결론이 흔들리는지**를 정리한 것이다. 답이 오면 [`configs/memories_default.yaml`](configs/memories_default.yaml)의 해당 필드만 고치고 `calc_attention.py`를 다시 돌리면 §4의 표가 갱신된다.

#### 우선순위 1 — 이 값이 없으면 결론을 낼 수 없다

| # | 확인 항목 | config 필드 | 현재 가정 | 모르면 흔들리는 결론 |
|:---:|---|---|---|---|
| 1 | **Custom HBM 노드의 Attention 연산 성능** (fp16/bf16 유효 TFLOPS) | `custom_hbm.compute_tflops_fp16` | 32 TFLOPS | §4.6 — 16이면 처리량 이득이 +34%에서 **+8%로 급감**. 이 DP의 논거 자체 |
| 2 | **Custom HBM 노드의 내부 집계 대역폭** (근접 연산 유닛이 실제로 쓸 수 있는 값) | `custom_hbm.int_bw_bytes_per_s` | 8.0 TB/s | §4.2의 균형점 44.8 TFLOPS가 이 값에 비례. 대역폭이 절반이면 필요 연산도 절반 |
| 3 | **CXL-PNM의 Attention 연산 성능** | `cxl_pnm.compute_tflops_fp16` | 16 TFLOPS | §4.4 — 6.2 TFLOPS 미만이면 CXL-PNM도 연산 한계로 떨어져 Decode 오프로드가 무의미 |
| 4 | **Attention 커널의 대역폭 달성률** (peak 대비 실측) | `*.attention_bw_efficiency` | 0.7 | 모든 Mode C 수치에 직접 곱해진다 |

> **1번이 가장 중요하다.** §4.6이 보여주듯 이 값 하나로 결론이 "이 구조를 채택할 만하다"와 "거의 이득 없다" 사이를 오간다. **요구할 숫자는 "최대한 많이"가 아니라 §4.2의 균형점(Custom HBM 44.8 / CXL-PNM 6.2 TFLOPS)** 이며, 그 이상은 대역폭이 병목이라 이득이 없다.
...
| 8 | **Scale-up fabric 종류와 방향당 대역폭·지연** | `custom_hbm.ext_bw_bytes_per_s`, `latency_s` | 0.9 TB/s / 1 μs | §4.4 — Prefill KV 스트리밍이 GPU 연산에 가려지는지가 여기 걸린다 |
...
> **§4의 모든 수치는 시뮬레이션 이전의 예측이며, 시뮬레이션으로 검증해야 하고, 시뮬레이션 자체도 위 가정값 위에 서 있다.** 따라서 현 단계 결론은 **"어떤 조건이 만족되면 이 구조가 값을 하는가"** 의 형태로만 인용해야 하며, 절대 수치를 성과로 인용하면 안 된다. 실제 값이 들어오면 §4를 재생성하고 §13의 조건부 선정에 반영한다.

---

## 4. Attention / FFN 분리 실행

§3.3에서 본 외부/내부 비대칭을 실제로 활용하는 실행 구조다. **KV를 경로로 끌어내지 않고, Attention 연산을 KV가 있는 메모리에서 수행한다.**

> **이 장의 모든 수치는 [`configs/calc_attention.py`](configs/calc_attention.py)가 [`configs/memories_default.yaml`](configs/memories_default.yaml)에서 직접 계산한 것이다.** 손으로 적은 값이 Configuration과 어긋나는 것을 막기 위함이며, config를 고치면 스크립트를 다시 돌려 표를 갱신한다.
...
KV는 경로를 건너지 않는다. 건너는 것은 Q, K_new/V_new, attn_out — 모두 **토큰 수 × hidden** 크기의 활성화 텐서다. 신규 토큰의 K/V도 같은 메모리에 append되므로 **Attention 전체가 한 곳에서 끝나고 softmax 재정규화가 필요 없다.**

### 4.2 연산 강도 — 무엇이 오프로드 가능한가를 결정한다

**메모리의 대역폭만 보고 오프로드를 판단하면 안 된다.** 근접 연산 유닛의 연산 성능이 부족하면 대역폭이 남아도 연산이 병목이 된다. Attention의 연산 강도는 다음과 같이 깔끔하게 정리된다.
...
가정 모델(H=64, KVH=8 → GQA 8:1)에서:

| 단계 | query token 수 | 연산 강도 | 성격 |
|---|---:|---:|---|
| **Decode** | 1 | **8 FLOP/B** | **대역폭 한계** — 오프로드에 적합 |
| **Incremental Prefill** | 512 | **4,096 FLOP/B** | **연산 한계** — 오프로드에 부적합 |

**대역폭 한계를 유지하려면 필요한 연산 성능** = 내부 대역폭 × 연산 강도.

| 메모리 | Decode에 필요 | Prefill에 필요 | 가정한 성능 | 판정 |
|---|---:|---:|---:|---|
| Custom HBM 노드 | **44.8 TFLOPS** | 22,938 TFLOPS | 32 TFLOPS (가정) | Decode 연산 한계 / Prefill 불가 |
| CXL-PNM | **6.2 TFLOPS** | 3,154 TFLOPS | 16 TFLOPS (가정) | Decode 대역폭 한계 / Prefill 불가 |

> **여기서 두 가지가 확정된다.**
>
> **(1) Incremental Prefill은 어떤 연산형 메모리로도 보낼 수 없다.** 필요 성능이 수천 TFLOPS로 GPU급이며, 가정한 성능으로 돌리면 1.4~2.8초가 걸린다(아래 §4.4). **Prefill은 GPU에 남는다.**
>
> **(2) Decode는 오프로드 대상이다.** 필요 성능이 수~수십 TFLOPS 수준이라 근접 연산 유닛의 사정거리 안에 있다. **44.8 TFLOPS가 Custom HBM 노드의 균형점**이며, 그 이상은 대역폭이 병목이 되어 더 넣어도 이득이 없다 — §3.6에서 HW팀에 확인할 핵심 수치다.

### 4.3 경로를 건너는 바이트와 계층당 왕복

```text
KV 크기       토큰·계층당 4 KiB → 토큰당 320 KiB → 32K context = 10.0 GiB
활성화        계층당 36 KiB (Q 16 + K/V append 4 + attn_out 16) → 토큰당 2.81 MiB
                                                          ↑ 약 3,600배 차이
```

왕복은 계층 수만큼(80회) 발생한다.

| 메모리 | 전송 | 왕복 지연 | 링크 오버헤드 |
|---|---:|---:|---:|
| Custom HBM 노드 | 3 μs | 80 μs | **83 μs / token** |
| CXL-PNM | 46 μs | 20 μs | **66 μs / token** |
| (SSD-PIM) | 176 μs | 4,800 μs | **4.98 ms / token — 불가** |

Decode step(수 ms) 대비 앞의 둘은 1% 미만이지만 SSD-PIM은 성립하지 않는다. 배치 안의 여러 요청이 같은 계층을 함께 처리하면 왕복이 상쇄되므로 **이 비용은 배치 크기에 반비례**한다.

### 4.4 단계별 실행 위치 — 계산 결과

#### Decode 1 step (세션 1개, KV 10 GiB)

| 경로 | Step 시간 | GPU 대비 | 한계 |
|---|---:|---:|---|
| **GPU 로컬 HBM** | **1.49 ms** | 1.00× | 대역폭 |
| Custom HBM 노드, fabric read (Mode A) | 11.93 ms | 8.00× | 경로 대역폭 |
| **Custom HBM 노드, Attention 오프로드** | **2.77 ms** | **1.86×** | **연산** (32 TFLOPS 가정) |
| CXL-PNM, Attention 오프로드 | 14.01 ms | 9.39× | 대역폭 |
| HBF, GPU 직접 read (Mode A) | 10.74 ms | 7.20× | 경로 대역폭 |

> **이전 판의 "오프로드하면 로컬 HBM과 대등(1.06×)"은 틀렸다.** 연산 성능 제약을 넣지 않고 대역폭만으로 계산했기 때문이다. 실제로는 **32 TFLOPS 가정에서 1.86× 느리다.** 균형점인 44.8 TFLOPS를 채워도 2.00 ms(1.34×)로 로컬 HBM보다는 느리다.
>
> **그래서 이 DP의 논거는 step latency가 아니다** — §4.5로 넘어간다.

#### Incremental Prefill (ΔT = 512)

| 경로 | 시간 | 비고 |
|---|---:|---|
| **GPU 로컬 HBM** | **43.98 ms** | **연산 한계.** 이것이 기준 |
| Custom HBM 노드에 오프로드 | 1,376 ms | 31.3× — **불가** |
| CXL-PNM에 오프로드 | 2,772 ms | 63.0× — **불가** |
| **Custom HBM 노드 → KV 스트리밍 + GPU 연산** | **43.98 ms** | 전송 11.93 ms가 **GPU 연산에 완전히 가려짐** |
| CXL-PNM → KV 스트리밍 + GPU 연산 | 167.77 ms | 전송이 지배 — **가려지지 않음** |

> **Custom HBM 노드의 재활성은 사실상 공짜다.** Prefill이 연산 한계(44 ms)라서, fabric으로 KV를 스트리밍하는 11.9 ms가 그 아래 완전히 숨는다. 스트리밍은 계층별로 읽고 버리므로 **HBM에 상주시키지 않는다** — 용량을 소비하지 않고 Prefill을 마친 뒤 Decode는 다시 오프로드로 돌아간다.
>
> **CXL-PNM은 그렇지 않다.** 전송 168 ms가 연산 44 ms를 4배 넘겨 숨지 않는다. 재활성이 잦은 세션을 CXL-PNM에 두면 안 되는 이유이며, 이 계층이 **재접근이 드문 KV의 자리**인 근거다.

### 4.5 처리량 관점 — 이 구조의 진짜 이득

**Attention 오프로드의 이득은 Latency가 아니라 처리량이다.** §4.4에서 보듯 오프로드된 Decode step은 GPU보다 느리다. 그럼에도 이득이 나는 이유는 **GPU와 연산형 메모리가 직렬이 아니라 병렬로 돌기 때문**이다.
...
GPU: [prefill][dec][dec]...[dec]   GPU : [prefill][w][w]...[w]
      연산한계  대역폭한계 (직렬)    노드:         [attn][attn]...[attn]
                                            └─ 두 자원이 병렬 ─┘
턴 시간 = prefill + N×(attn+weight)  턴 시간 = max(GPU, 노드)
```

**Prefill은 연산 한계라 GPU가 잘하고, Decode attention은 대역폭 한계라 메모리가 잘한다.** 서로 다른 자원을 쓰므로 겹칠 수 있다 — 이것이 이기종 메모리에 연산 기능을 넣는 이유다.

가정 모델 + Custom HBM 노드(32 TFLOPS 가정), 가중치 read 2.2 ms/step(TP8 가정) 기준:

| N (턴당 생성 토큰) | GPU만 | 오프로드 | 처리량 변화 | 병목 |
|---:|---:|---:|---:|---|
| 8 | 73.5 ms | 61.6 ms | **+19.4%** | GPU |
| **20** (Agent 전형) | 117.8 ms | 88.0 ms | **+33.9%** | GPU |
| 50 | 228.5 ms | 154.0 ms | **+48.4%** | GPU |
| 143 | 571.8 ms | 395.8 ms | +44.5% | 노드 |
| 400 | 1520.5 ms | 1107.1 ms | +37.3% | 노드 |

여기에 **용량 이득이 더해진다.** GPU HBM 192 GiB는 10 GiB 세션을 19개 담지만 Custom HBM 노드 1 TiB는 102개를 담는다. 동시 수용 세션이 5배가 되어 배치를 더 키울 수 있고, 이는 가중치 read를 더 넓게 분산시킨다.

> **그래서 §11의 주 지표가 M-P1(Goodput)이고 M-P3(TPOT)은 보조다.** Step latency만 보면 이 구조는 항상 손해로 보인다. **자원 병렬화와 용량 확대가 처리량으로 환산되는 경로를 목적함수가 담지 못하면 이 DP의 논거 자체가 측정되지 않는다.**

### 4.6 민감도 — 이 결론이 기대는 가정

노드의 연산 성능이 결론을 지배한다. N=20 기준, Custom HBM 노드의 TFLOPS만 바꾸면:

| 노드 TFLOPS | Decode step | 턴 시간 | 처리량 변화 | 한계 |
|---:|---:|---:|---:|---|
| 16 | 5.45 ms | 109.0 ms | **+8.0%** | 연산 |
| **32** (현 가정) | 2.77 ms | 88.0 ms | **+33.9%** | 연산 |
| **44.8** (균형점) | 2.00 ms | 88.0 ms | +33.9% | 대역폭 |
| 64 | 2.00 ms | 88.0 ms | +33.9% | 대역폭 |
| 128 | 2.00 ms | 88.0 ms | +33.9% | 대역폭 |

**세 가지를 읽어야 한다.**

1. **16 TFLOPS면 이득이 +8%로 줄어든다.** 연산 성능이 부족하면 이 구조의 논거가 거의 사라진다.
2. **44.8 TFLOPS에서 포화한다.** 그 이상은 대역폭이 병목이라 넣어도 소용없다. **HW팀에 요구할 숫자는 "최대한 많이"가 아니라 "44.8 TFLOPS"다.**
3. **포화 이후 턴 시간이 88.0 ms로 고정되는 것은 병목이 GPU로 넘어갔기 때문**이다. 이 지점부터는 노드를 개선해도 소용없고 GPU 쪽 Prefill이 한계다.

### 4.7 성립 조건

| 조건 | 내용 |
|---|---|
| **원시 연산 지원** | `QK_GEMM`, `SOFTMAX`, `AV_GEMM`, `CAUSAL_MASK`를 모두 지원해야 한다. `ssd_pim`은 `SOFTMAX`가 없어 제외된다 |
| **연산 성능** | §4.2의 균형점(Custom HBM 44.8 / CXL-PNM 6.2 TFLOPS)에 근접해야 한다. 부족하면 §4.6대로 이득이 급감한다 |
| **지연 예산** | 계층당 왕복 × 계층 수가 TPOT 예산 안에 들어와야 한다. SSD-PIM(4.98 ms/token)은 불가 |
| **단계 분리** | **Prefill은 GPU, Decode attention만 오프로드**. 이 분리가 없으면 §4.4대로 Prefill이 31~63배 느려진다 |
| **용량** | 연산형 메모리가 모든 세션을 담을 수 없다. **선택받은 일부만** 갈 수 있다 — 배치 정책이 필요한 이유 |
| **재사용 확률** | 세션이 돌아오지 않으면 자리를 낭비한 것이다. §5.1의 Reuse Probability 추정이 필요하다 |
...
Mode A. Resident        GPU가 직접 읽는 메모리에 유지 (HBM, Custom HBM 노드, HBF)
                        → 재활성 시 복원 0. 단 Decode마다 그 메모리의 **외부 대역폭**으로
                          ...
                          (§4.4: Custom HBM 노드 fabric read = 로컬 HBM의 8.9배)
...
4. **§4.5의 자원 병렬화가 나타나는 유일한 지표다.** Attention 오프로드는 Step latency 기준으로는 항상 손해로 보인다(§4.4에서 1.86× 느림). 이득은 **GPU와 연산형 메모리가 병렬로 도는 것**과 **용량 확대로 배치가 커지는 것**에서 나오며, 이 둘은 처리량으로만 환산된다.
...
> **주 지표가 아니다.** §4.4에서 보듯 Attention 오프로드는 TPOT를 **악화**시킨다(1.86×). 그럼에도 §4.5의 처리량은 +34% 개선된다. **TPOT만 보고 판단하면 이 DP의 결론이 정반대로 나온다.** TPOT는 SLO 제약(M-P1)을 만족하는지 확인하고 오프로드가 대화형 예산을 넘지 않는지 보는 용도로 쓴다.
...
**병목이 어디인지를 함께 보고한다.** §4.6에서 보듯 노드 연산 성능이 균형점을 넘으면 병목이 GPU로 넘어가고, 그 지점부터는 메모리를 개선해도 처리량이 오르지 않는다. **이 전환점이 관측되지 않으면 어느 자원에 투자해야 하는지 답할 수 없다.**
...
| **연산형 메모리의 Attention 연산 성능 (TFLOPS)** | **가장 중요한 축.** §4.6대로 16 TFLOPS면 처리량 이득이 +34%에서 +8%로 떨어진다. §4.2의 균형점(44.8 / 6.2 TFLOPS)을 걸쳐서 훑는다 |
| **Custom HBM 노드 내부 대역폭** | §3.5의 가정값(8.0 TB/s). §4.2의 균형점이 이 값에 비례한다 |
| **턴당 생성 토큰 수 N** | §4.5의 처리량 이득이 N에 따라 +19%~+48%로 변하고 병목 자원도 바뀐다 |
| **Scale-up fabric 세대 (NVLink 5 / 차세대 / UALink 고밀도)** | Custom HBM 노드의 외부 대역폭. 비대칭 8.9×가 줄면 Mode A/C 격차가 줄어든다 |
...
| **GPU Occupancy 실측 (M-P7b)** | §4.5의 자원 병렬화가 이 DP의 핵심 논거다. 시뮬레이션은 자원별 점유 시간으로 대리 측정하지만, 실제 커널 스케줄링에서 GPU와 메모리가 정말 겹치는지는 실측해야 한다 |
