...
LLM/Agent 추론 시스템 내에서 HBM, DRAM, CXL-PNM, Custom HBM, HBF, SSD-PIM 등 저장·연산 특성이 서로 다른 메모리가 혼재할 때, **KV 캐시를 어느 메모리에 둘 것인지**를 어떤 기준으로 결정할지 설계한다.
...
Agent 세션 하나의 수명을 따라가면 배치를 결정해야 하는 지점이 **두 번** 나온다.

```text
   ┌─────────────────────────── 한 턴 ───────────────────────────┐

   Prefill  ──►  [ 결정점 A ]  ──►  Decode  ──►  [ 결정점 B ]  ──►  유휴  ──┐
   (GPU 고정)   "Decode를 어디서?"              "유휴 중 어디에?"        │
                                                                        │
   ◄──────────────────  재활성 (Incremental Prefill, GPU)  ──────────────┘
```

| | **결정점 A — Prefill 종료 시점** | **결정점 B — 비활성 전환 시점** |
|---|---|---|
| **질문** | 이번 턴의 **Decode Attention을 어디서 수행할 것인가** | 유휴 구간 동안 **KV를 어디에 보관할 것인가** |
| **대상** | 방금 Prefill이 만든 KV + 누적 History | 같은 세션의 전체 KV |
| **후보** | **Decode Attention을 감당할 수 있는 메모리만** — HBM / Custom HBM 노드 / CXL-PNM | **전체 6종** — 보관만 하면 되므로 연산 기능이 없어도 된다 |
| **주된 판단 근거** | 이번 턴의 예상 Decode 길이, 세션 KV 크기, 메모리의 Attention 수행 능력 | 재접근 시점·확률, 잔여 수명 (§5.1) |
| **틀렸을 때** | Decode 내내 비싼 경로로 Attention을 돌린다 | 재활성 시 복원 비용을 물거나, 유휴 KV가 빠른 계층을 점유한다 |

#### 왜 Prefill 종료 시점이 진짜 결정점인가

**Prefill은 GPU에 고정된다.** §4.2에서 다루듯 Prefill Attention은 연산 강도가 높아 근접 연산 유닛으로 보낼 수 없다. **따라서 연산형 메모리를 활용할 여지는 Decode 구간에만 있고, 그 여부를 정하는 시점이 Prefill이 끝나는 순간이다.**

여기서 선택지가 실재하는 이유는 **연산형 메모리는 되가져올 필요가 없기 때문**이다.

```text
연산 기능이 없는 메모리 (DRAM / HBF / SSD-PIM)
   KV를 여기 쓰면 → Decode 때 다시 GPU로 끌어와야 함 → 순손실
   → 결정점 A의 후보가 아니다

연산 기능이 있는 메모리 (Custom HBM 노드 / CXL-PNM)
   KV를 여기 쓰면 → Decode Attention을 그 자리에서 수행 → 되가져오지 않음
   → 결정점 A의 실제 후보다
```

> **이전 판에서 "갓 생성된 KV를 하위 계층에 두는 것은 순손실이므로 배치 결정은 비활성 KV에만 실재한다"고 서술했으나, 이는 연산 기능이 없는 계층만 고려한 결론이었다.** 연산형 메모리가 있으면 활성 Decode 구간의 KV 위치도 실제 선택이 된다. 본 판에서 결정점 A를 추가한다.

### 대상 범위

| | |
|---|---|
| **대상** | 결정점 A·B에서의 KV 캐시 배치 및 이후의 재배치 |
| **대상 아님** | **Prefill 연산의 실행 위치** — §4.2에 따라 GPU로 고정된다. 어느 *Node*에서 Prefill을 할지는 DP2의 쟁점 |
...
| **선점 (Preemption)** | 상위 계층 부족으로 Request가 밀려남 | 재개 시 | 매우 높음 |
| **세션 종료 후 보관** | Request 완료 | 이후 세션의 Prefix Cache Hit | 낮음 |

> **본 문서는 설계와 그 설계가 서 있는 가정을 명세한다.** 각 후보가 어떤 성능을 낼 것인가는 별도의 시뮬레이션으로 측정하며, 그 결과는 별도 문서에서 다룬다. 시뮬레이션을 정확히 수행하기 위해 확인해야 할 HW 스펙은 §3.6에 정리한다.
...
### 3.0 노드 구성 — 무엇이 어디에 붙어 있는가

대역폭 수치를 읽기 전에 **물리적 배치**를 고정해야 한다. 같은 메모리라도 어디에 붙어 있는지에 따라 도달 경로의 대역폭이 자릿수로 달라진다.
...
```text
   ...
   │  CPU ── DRAM   1 TiB  │
   │   ║                   │
   │   ╠══ CXL ══ CXL-PNM  512 GiB  (내부 1.1 TB/s / 외부 64 GB/s)
   │   ╚══ NVMe ═ SSD-PIM   16 TiB
   └───────────────────────┘
```

세 가지 경로가 있고 대역폭이 자릿수로 다르다.

| 경로 | 홉 | 대역폭 급 |
|---|---|---|
| **on-package** (HBM, HBF) | 0 | TB/s 급 |
| **scale-up fabric** (Custom HBM 노드) | 1 | 수백 GB/s ~ TB/s |
| **CPU 경유** (DRAM, CXL-PNM, SSD-PIM) | 2 | 수십 GB/s |

> **Custom HBM은 GPU 패키지 안이 아니라 옆의 별도 노드다.** Custom HBM만으로 구성되어 GPU-HBM을 대체하며, 연산 다이가 면적을 다투지 않으므로 **용량이 GPU HBM보다 훨씬 크다.** 이 배치가 §3.2와 §4의 결론을 지배한다.

### 3.1 6종 메모리와 연산 가능 여부

| 메모리 | 위치 | 연산 가능 | 용량 | 외부 BW | 내부 BW | GPU 직접 접근 |
|---|---|:---:|---:|---:|---:|:---:|
| **HBM** | GPU on-package | ✗ | 192 GiB | 8.0 TB/s | — | ✓ |
| **Custom HBM** | **별도 노드 (fabric)** | **✓** | **1.00 TiB** | 0.9 TB/s | **8.0 TB/s** | ✓ (fabric) |
| **CXL-PNM** | CPU 측 (CXL) | **✓** | 512 GiB | 0.064 TB/s | **1.1 TB/s** | ✗ |
| **DRAM** | CPU 측 | ✗ | 1.00 TiB | 0.064 TB/s | — | ✗ |
| **HBF** | GPU on-package (UCIe) | ✗ | 2.00 TiB | 1.0 TB/s | — | ✓ |
| **SSD-PIM** | CPU 측 (NVMe) | **✓** | 16.00 TiB | 0.016 TB/s | 0.2 TB/s | ✗ |

연산 가능 메모리는 **Custom HBM, CXL-PNM, SSD-PIM** 세 종이다. HBF는 대역폭이 높지만 연산 기능이 없으므로 **용량 계층**으로만 쓴다.

### 3.2 Interconnect Exploration

배치 결정에서 결정적인 것은 메모리 자체의 대역폭이 아니라 **그 메모리에 도달하는 경로의 대역폭**이다.

#### Custom HBM 노드 ↔ GPU — scale-up fabric

별도 노드이므로 on-package가 아니라 **scale-up fabric 한 홉**을 탄다. 현재 이 자리를 차지하는 것은 NVLink와 UALink다.

```text
   GPU ══[ scale-up fabric, 1 hop ]══► Custom HBM 노드
          방향당 0.9 TB/s                 1 TiB
                                          내부 8.0 TB/s
```
...
| | 값 | 근거 |
|---|---|---|
| NVLink 5 | GPU당 **1.8 TB/s** 양방향 = **방향당 900 GB/s** (18 link × 100 GB/s) | 공개 제품 수치 |
| UALink 1.0 | lane당 200 Gbps(~25 GB/s). lane 수와 토폴로지로 대역을 맞춤 | 규격 |
| (참고) 고밀도 구성 | 가속기당 ~3.6 TB/s 집계 사례 | 벤더 발표 |
| 노드 내부 대역폭 | **~8.0 TB/s** | **가정값** — 노드 내 HBM 스택 집계 |
| 용량 | **~1 TiB** | **가정값** — GPU HBM의 약 5배 |
| fabric 지연 | ~1 μs | **가정값** — on-package(300 ns)보다 큼 |

- **외부/내부 비대칭 ≈ 8.9배.** on-package를 가정했을 때(~1.5배)와 완전히 다른 그림이다.
- **함의가 바뀐다.** 이전 판에서는 "Custom HBM의 오프로드 이득은 대역폭이 아니라 GPU Occupancy뿐"이라고 썼으나, **별도 노드에서는 그렇지 않다.** fabric으로 KV를 읽으면 로컬 HBM 대비 **8.9배 느리고**, 노드 안에서 Attention을 수행하면 **로컬 HBM과 대등한 처리량**을 낸다(§4.4). 즉 이 노드에서 **Attention 오프로드는 선택이 아니라 사실상 전제**다.

#### CXL-PNM ↔ GPU — CPU 경유 두 홉

```text
   GPU ──[ PCIe ]── CPU ──[ CXL ]── CXL-PNM
                                    LPDDR5X, 512 GiB
        └──────── 64 GB/s ───────┘  내부 1.1 TB/s
                (링크가 병목)
```
...
| | 값 | 근거 |
|---|---|---|
| CXL 링크 (PCIe 5.0 x16) | 방향당 **64 GB/s** | PCIe 5.0 규격 |
| (참고) PCIe 6.0 / 7.0 x16 | 128 / 256 GB/s | 세대별 목표치 |
| 장치 내부 대역폭 | **~1.1 TB/s** | LPDDR5X 기반 CXL-PNM 플랫폼 공개값 |
| 용량 | ~512 GB | 동상 |
| 링크 지연 | ~170~250 ns, CPU 홉 포함 시 더 큼 | CXL.mem 공개 측정 범위 |

- **외부/내부 비대칭 ≈ 17배**로 이 구성에서 가장 크다. KV를 링크로 끌어내는 순간 1.1 TB/s짜리 메모리를 64 GB/s로 쓰는 셈이 된다.
- **함의:** CXL-PNM에 둔 KV는 옮기면 안 되고 연산을 보내야 한다. 단 내부 대역폭 자체가 HBM보다 낮으므로 **오프로드해도 로컬 HBM보다는 느리다**(§4.4). 이 계층의 값어치는 속도가 아니라 **용량당 비용**이다.

#### HBF ↔ GPU — on-package UCIe

2026년 FMS에서 SK hynix·SanDisk가 **OCP 기술 규격 초판**을 공개했다.
...
- GPU가 직접 읽을 수 있고 대역폭도 1 TB/s 급이지만, **로컬 HBM(8 TB/s)보다는 8배 느리다.** 매체가 NAND이므로 읽기 지연도 μs 급이고 **쓰기 대역폭과 Write Endurance가 제약**이다.
- **함의:** 연산 기능이 없으므로 §1 결정점 A의 후보가 아니다. 외부 대역폭이 CPU 측 계층보다 한참 높아 **대용량 보관 + 빠른 복원**이 이 계층의 역할이며, 결정점 B에서 재활성이 잦은 KV를 받을 수 있다. 다만 **왕복이 반복되면 수명이 소모**되므로(§11.5 M-E1) 그 한계를 함께 봐야 한다.

#### SSD-PIM ↔ GPU — CPU 경유 NVMe

| | 값 |
|---|---|
| 호스트 링크 (PCIe 5.0 x4) | ~16 GB/s |
| 내부 NAND 집계 대역폭 | ~200 GB/s (가정값) |
| 지연 | 수십 μs 급 |
| 지원 원시 연산 | `QK_GEMM`, `AV_GEMM` — **`SOFTMAX` 없음** |

- `SOFTMAX`가 없어 **Attention을 여기서 끝낼 수 없다.** 오프로드 대상이 아니다(§4.5).
- 지연이 μs 급이라 계층당 왕복 × 계층 수가 대화형 경로 예산을 넘는다(§4.3).
- **함의:** 재사용 확률이 낮은 장기 보관용.

### 3.3 외부/내부 대역폭 비대칭 — 연산 오프로드 판단의 1차 기준

| 메모리 | 외부(경로) BW | 내부 BW | 비대칭 | 연산 오프로드 |
|---|---:|---:|---:|---|
| HBM | 8.0 TB/s | = 외부 | 1.0× | 불가 (연산 기능 없음) |
| **Custom HBM** | **0.9 TB/s** | **8.0 TB/s** | **8.9×** | **가능** — 비대칭이 크므로 끌어내지 말고 연산을 보내는 쪽이 유리 |
| **CXL-PNM** | **0.064 TB/s** | **1.1 TB/s** | **17.2×** | **가능** — 이 구성에서 비대칭이 가장 크다 |
| DRAM | 0.064 TB/s | 0.4 TB/s | 6.2× | 불가 (연산 기능 없음) |
| HBF | 1.0 TB/s | = 외부 | 1.0× | 불가 (연산 기능 없음) |
| SSD-PIM | 0.016 TB/s | 0.2 TB/s | 12.5× | 불가 (`SOFTMAX` 미지원) |

> **비대칭이 큰 메모리일수록 "KV를 옮길지 연산을 보낼지"의 답이 명확해지며, 그 판단을 하려면 이 KV가 다음에 어떤 연산을 받는지를 알아야 한다** — 그것이 데이터 특성 기반 후보(C2)의 고유 입력이다(§7).
>
> **비대칭이 크다고 좋은 계층인 것은 아니다.** SSD-PIM은 12.5배지만 `SOFTMAX`가 없어 오프로드가 불가능하고, DRAM은 6.2배지만 연산 기능이 없다. **비대칭은 "오프로드가 값을 하는가"를 말하고, 절대 대역폭이 "그 계층이 빠른가"를 말한다.** 두 축을 함께 봐야 한다.
...
**원본을 YAML로 둔 이유는 엄격한 JSON에 주석 문법이 없기 때문이다.** 이 값들은 단위(`# 512 GiB`)와 출처(`src: SPEC / PUBLIC / ASSUMED`)가 값 옆에 보여야 편집 실수를 막을 수 있다. JSON 쪽에서도 `provenance`·`note`를 데이터 필드로 유지하고 `units` 블록을 덧붙인다. `pyyaml`은 vLLM 공통 의존성이므로 추가 설치가 필요 없다.
...
- name: custom_hbm
  ...
  capacity_bytes:    1099511627776   # 1.00 TiB   src: ASSUMED GPU HBM의 약 5배
  ext_bw_bytes_per_s:       9.0e+11  # 0.9 TB/s   src: SPEC    NVLink 5 방향당 900 GB/s
  int_bw_bytes_per_s:       8.0e+12  # 8.0 TB/s   src: ASSUMED 노드 내부 HBM 스택 집계
  latency_s:                1.0e-06  # 1 us       src: ASSUMED scale-up fabric 한 홉
  hops: [gpu->dev:scaleup_fabric]
  gpu_reachable: true
...
**`ext_bw`와 `int_bw`를 분리한다.** 하나로 합치면 §3.3의 비대칭이 표현되지 않고, **연산 오프로드가 왜 값을 하는지가 모델에서 사라진다.** 연산 가능 메모리는 `int_bw`를 반드시 선언해야 하며, 없으면 loader가 거부한다.

**`supported_primitives`가 연산 단위가 아니라 원시 연산 단위다.** "GEMV를 지원한다"와 "Attention을 처리할 수 있다"는 다르다. `ssd_pim`은 `SOFTMAX`가 없어 오프로드 대상이 되지 않는다.

**`hops`가 경로를 명시한다.** §3.0의 세 경로(on-package / fabric / CPU 경유)가 `hops`로 구분되며, 홉 수가 실효 대역폭과 지연을 바꾼다.

파일 최상단에는 **시뮬레이션 입력으로 쓸 기준 GPU와 모델 형상**도 함께 둔다(`gpu:`, `model:`). §4.2의 연산 강도가 `num_heads / num_kv_heads`에서 나오므로 모델 형상이 오프로드 가능 여부를 바꾸며, 모델을 교체하면 결론도 달라진다 — 이 역시 시뮬레이션에서 훑을 축이다.

### 3.5 수치의 출처와 가정

| 항목 | 출처 | 상태 |
|---|---|---|
| HBM 8-stack 집계 대역폭, HBM4 스택당 2.0~3.3 TB/s | 공개 스펙·벤더 발표 | 공개값 |
| **NVLink 5 방향당 900 GB/s (1.8 TB/s 양방향)** | 공개 제품 수치 | **공개값** |
| UALink 1.0 lane당 200 Gbps | 규격 | 공개값 |
| **Custom HBM 노드 용량 1 TiB** | — | **가정값** |
| **Custom HBM 노드 내부 대역폭 8.0 TB/s** | — | **가정값** (공개 정량값 없음) |
| **Custom HBM fabric 지연 1 μs** | — | **가정값** |
| HBF 스택 512 GB, 0.4~3.0 TB/s, UCIe | OCP 기술 규격 초판 (FMS 2026) | 공개값 |
| HBF 쓰기 대역폭·지연·Endurance Budget | — | **가정값** |
...
> **가정값이 결론을 지배하지 않는지 확인해야 한다.** §11.6의 Sweep에 **Custom HBM 노드의 내부 대역폭과 fabric 세대**, **HBF 쓰기 특성**을 반드시 포함한다. 특히 Custom HBM 노드의 내부 대역폭은 §4.4의 결론("오프로드가 로컬 HBM과 대등")을 직접 만드는 값이므로, 이 값이 절반이면 결론이 달라진다.

### 3.6 HW팀 확인 항목 — POC 전에 채워야 할 값

§3.5의 가정값은 시뮬레이션을 돌리기 위한 잠정값이다. **실제 수치가 들어와야 시뮬레이션 결과를 POC에 연결할 수 있다.** 아래는 **무엇을 물어야 하고, 모르면 어느 결론이 흔들리는지**를 정리한 것이다. 답이 오면 [`configs/memories_default.yaml`](configs/memories_default.yaml)의 해당 필드만 고치면 되고, 코드 수정은 필요 없다.

#### 우선순위 1 — 이 값이 없으면 결론을 낼 수 없다

| # | 확인 항목 | config 필드 | 현재 가정 | 모르면 흔들리는 결론 |
|:---:|---|---|---|---|
| 1 | **Custom HBM 노드의 Attention 연산 성능** (fp16/bf16 유효 TFLOPS) | `custom_hbm.compute_tflops_fp16` | 32 TFLOPS | §4.2의 균형점에 못 미치면 연산 한계로 떨어져 **오프로드 이득이 급감**한다. 이 DP의 논거 자체 |
| 2 | **Custom HBM 노드의 내부 집계 대역폭** (근접 연산 유닛이 실제로 쓸 수 있는 값) | `custom_hbm.int_bw_bytes_per_s` | 8.0 TB/s | §4.2의 균형점이 이 값에 비례한다. 대역폭이 절반이면 필요 연산 성능도 절반 |
| 3 | **CXL-PNM의 Attention 연산 성능** | `cxl_pnm.compute_tflops_fp16` | 16 TFLOPS | 균형점 미만이면 연산 한계로 떨어져 Decode 오프로드가 무의미해진다 |
| 4 | **Attention 커널의 대역폭 달성률** (peak 대비 실측) | `*.attention_bw_efficiency` | 0.7 | 모든 Mode C 수치에 직접 곱해진다 |

> **1번이 가장 중요하다.** 이 값 하나로 결론이 "이 구조를 채택할 만하다"와 "거의 이득 없다" 사이를 오간다. **요구할 숫자는 "최대한 많이"가 아니라 §4.2의 균형점(`내부 대역폭 × GQA 비율`)** 이며, 그 이상은 대역폭이 병목이라 이득이 없다. 2번이 확정되면 이 값이 계산된다.

#### 우선순위 2 — 구조 결정에 영향

| # | 확인 항목 | config 필드 | 현재 가정 | 왜 필요한가 |
|:---:|---|---|---|---|
| 5 | **지원 원시 연산** — Softmax(exp/max/reduction), Causal Mask, GQA head 매핑, RoPE를 장치가 직접 처리하는가 | `*.supported_primitives` | Custom HBM·CXL-PNM은 4종 지원 가정 | 하나라도 빠지면 Attention이 그 메모리에서 끝나지 않아 **오프로드 자체가 불가** (§4.7) |
| 6 | **지원 정밀도** — fp16 / bf16 / fp8. Softmax 누적은 fp32인가 | (신규 필드 필요) | fp16 가정 | fp8 지원 시 KV 크기가 절반이 되어 §4의 모든 수치가 바뀐다 |
| 7 | **GPU가 memory semantics로 직접 load/store 가능한가** | `custom_hbm.gpu_reachable` | true | false면 Mode A(fabric read)가 불가능해지고 오프로드가 유일한 경로가 된다 |
| 8 | **Scale-up fabric 종류와 방향당 대역폭·지연** | `custom_hbm.ext_bw_bytes_per_s`, `latency_s` | 0.9 TB/s / 1 μs | §4.5 — 재활성 시 KV 스트리밍이 GPU 연산에 가려지는지가 여기 걸린다. 지연은 §4.3의 계층당 왕복에도 들어간다 |
| 9 | **동시 서비스 가능 세션 수 / 연산 유닛 병렬도** | (신규 필드 필요) | 무제한 가정 | 배치가 커질 때 노드가 선형으로 확장되는지. 안 되면 §4.5의 처리량 이득이 배치 크기에서 무너진다 |
| 10 | **CXL-PNM에 GPU가 P2P DMA로 직접 접근 가능한가** (CPU 홉 우회) | `cxl_pnm.hops` | CPU 경유 2홉 | 우회 가능하면 외부 대역폭이 올라 §4.4의 재활성 비용(168 ms)이 크게 낮아진다 |

#### 우선순위 3 — 용량·수명 계층

| # | 확인 항목 | config 필드 | 현재 가정 |
|:---:|---|---|---|
| 11 | Custom HBM 노드의 용량 | `custom_hbm.capacity_bytes` | 1 TiB |
| 12 | HBF의 쓰기 대역폭 | `hbf.write_bw_bytes_per_s` | 50 GB/s |
| 13 | HBF의 Write Endurance Budget과 Write Amplification | `hbf.endurance_budget_bytes`, `write_amplification` | 100 PB / 3.0 |
| 14 | HBF와 HBM이 같은 패키지에서 면적·전력을 나눠 쓰는가 (trade-off 관계인가) | (구성 자체) | 독립 가정 |
| 15 | SSD-PIM의 Softmax 지원 여부 | `ssd_pim.supported_primitives` | 미지원 |
| 16 | 각 메모리의 전력·열 제약 하 지속 성능 (peak 아닌 sustained) | (신규 필드 필요) | 고려 안 함 |

> **14번을 별도로 묻는 이유:** 현재 구성은 HBM 192 GiB와 HBF 2 TiB를 독립적으로 두고 있다. 같은 패키지에서 자원을 다투는 관계라면 **"HBM을 줄이고 HBF를 늘린 구성"** 을 §11.6의 Sweep 축으로 넣어야 한다.

#### 답이 오기 전까지의 취급

> **본 문서는 성능 수치를 담지 않는다.** §3.4의 가정값은 시뮬레이션의 입력이며, 어떤 구성에서 무엇이 얼마나 나오는지는 시뮬레이션이 답한다. 실제 HW 값이 들어오면 Configuration만 교체하면 되고, 시뮬레이션 결과는 별도 문서에서 다룬다. 현 단계의 결론은 **"어떤 조건이 만족되면 이 구조가 성립하는가"**(§4.4)의 형태로만 인용해야 한다.

---

## 4. Attention / FFN 분리 실행

§3.3에서 본 외부/내부 비대칭을 활용하는 실행 구조다. **KV를 경로로 끌어내지 않고, Attention 연산을 KV가 있는 메모리에서 수행한다.** 이 구조가 성립하는 범위가 곧 §1 결정점 A의 후보 집합을 정한다.

> **본 장은 구조와 그 성립 조건만 다룬다.** 각 구성에서 어떤 성능이 나오는지는 시뮬레이션의 몫이며 본 문서에 수치를 기재하지 않는다.
...
**KV는 경로를 건너지 않는다.** 건너는 것은 Q, K_new/V_new, attn_out — 모두 **토큰 수 × hidden** 크기의 활성화 텐서이며, KV보다 세 자릿수 작다. 신규 토큰의 K/V도 같은 메모리에 append되므로 **Attention 전체가 한 곳에서 끝나고 softmax 재정규화가 필요 없다.**

### 4.2 연산 강도 — Prefill과 Decode를 가르는 근거

**메모리의 대역폭만 보고 오프로드를 판단하면 안 된다.** 근접 연산 유닛의 연산 성능이 부족하면 대역폭이 남아도 연산이 병목이 된다. Attention의 연산 강도는 다음과 같이 정리된다.

```text
Attention FLOPs (QK^T + AV) = 4 · H · HD · S · q          (multiply-add = 2 FLOP)
KV bytes                    = 2 · KVH · HD · B · S

                                4 · H          H
연산 강도 = FLOPs / bytes  =  ───────── · q = ──── · q   [FLOP/byte]
                              4 · KVH         KVH

            = (GQA 비율) × (query token 수)
```

**이것은 성능 예측이 아니라 Attention 연산 자체의 성질이다.** query token 수가 곧 연산 강도의 배수이므로, 두 단계가 구조적으로 갈린다.

| 단계 | query token 수 | 연산 강도 | 성격 | 실행 위치 |
|---|---|---|---|---|
| **Decode** | 1 | **GQA 비율** (예: 8:1이면 8 FLOP/B) | **대역폭 한계** | **오프로드 가능** |
| **Incremental Prefill** | ΔT (수백~수천) | **GQA 비율 × ΔT** | **연산 한계** | **GPU 고정** |

**대역폭 한계를 유지하려면 필요한 연산 성능** = `내부 대역폭 × 연산 강도`.

> **여기서 두 가지가 확정된다.**
>
> **(1) Prefill은 어떤 연산형 메모리로도 보낼 수 없다.** 연산 강도가 query token 수에 비례해 커지므로 필요 연산 성능이 GPU급이 된다. 근접 연산 유닛의 규모로는 감당할 수 없다. **Prefill은 GPU에 남는다.**
>
> **(2) Decode는 오프로드 대상이다.** 연산 강도가 GQA 비율(상수)이므로 필요 연산 성능이 근접 연산 유닛의 사정거리 안에 들어올 수 있다.
>
> **(3) 따라서 연산형 메모리를 활용할 여지는 Decode 구간뿐이고, 그 여부를 정하는 시점이 §1의 결정점 A다.**

**요구 연산 성능의 상한이 정해진다는 점이 중요하다.** 위 식에 따라 `내부 대역폭 × GQA 비율`을 넘는 연산 성능은 대역폭이 병목이 되어 이득이 없다. **HW팀에 요구할 숫자는 "최대한 많이"가 아니라 이 균형점이며**, 실제 값은 §3.4 Configuration이 채워지면 계산된다 — §3.6의 확인 항목 1·2·3번이 이것이다.

### 4.3 경로를 건너는 비용의 구조

오프로드의 비용은 **두 성분**으로 나뉘며, 성질이 다르므로 분리해서 모델링해야 한다.

```text
계층당 1회 왕복 × 계층 수(L)

  (i) 전송 시간  = 활성화 bytes / 외부 대역폭
      → 활성화는 KV보다 세 자릿수 작으므로 작다

 (ii) 왕복 지연  = L × 경로 지연
      → 계층 수만큼 누적된다. 경로 지연이 큰 매체에서 지배적이 된다
      → 배치 안의 여러 요청이 같은 계층을 함께 처리하면 상쇄되므로
        배치 크기에 반비례한다
```

**(ii)가 오프로드 가능 여부를 가르는 두 번째 기준이다.** 지연이 μs 급인 매체는 `L × 지연`이 Decode step 예산을 넘어 대화형 경로에 넣을 수 없다. §3.4 구성에서 `ssd_pim`이 이 경우다.

### 4.4 Decode 오프로드의 성립 조건

§1 결정점 A에서 어떤 메모리가 후보가 되는지를 정하는 조건이다. **네 가지를 모두 만족해야 한다.**

| 조건 | 내용 | 확인 |
|---|---|---|
| **원시 연산 지원** | `QK_GEMM`, `SOFTMAX`, `AV_GEMM`, `CAUSAL_MASK`를 모두 지원해야 한다. 하나라도 빠지면 Attention이 그곳에서 끝나지 않는다 | §3.4 `supported_primitives` / §3.6 항목 5 |
| **연산 성능** | §4.2의 균형점(`내부 대역폭 × GQA 비율`)에 근접해야 한다. 크게 못 미치면 연산 한계로 떨어져 이득이 사라진다 | §3.6 항목 1·3 |
| **지연 예산** | `계층 수 × 경로 지연`이 Decode step 예산 안에 들어와야 한다 | §3.4 `latency_s` / §3.6 항목 8 |
| **용량** | 연산형 메모리가 모든 세션을 담을 수 없다. **선택받은 일부만** 갈 수 있다 — 배치 정책이 필요한 이유 | §3.6 항목 11 |

§3.4 기본 구성에서 이 조건을 만족하는 것은 **Custom HBM 노드와 CXL-PNM** 두 종이다. `ssd_pim`은 `SOFTMAX` 미지원과 지연 두 가지 모두에서 탈락한다.

### 4.5 재활성 경로 — Prefill이 GPU에 고정된 결과

결정점 B에서 하위 계층으로 내려간 KV가 다시 필요해지면, §4.2(1)에 따라 **Prefill은 GPU에서 수행해야 하므로 KV가 GPU 쪽으로 와야 한다.** 두 가지 경로가 있다.

```text
경로 1. 복원 (Restore)
   KV를 상위 계층으로 옮긴 뒤 GPU가 Prefill
   → 비용 = KV bytes / 외부 대역폭
   → 옮긴 KV가 상위 계층 용량을 점유한다

경로 2. 스트리밍 (Streaming)
   계층별로 읽어 GPU가 연산하고 버린다
   → 비용 = max(KV bytes / 외부 대역폭, GPU Prefill 연산 시간)
   → 상위 계층 용량을 점유하지 않는다
   → Prefill이 연산 한계(§4.2)이므로, 외부 대역폭이 충분하면
     전송이 연산 아래로 숨어 추가 비용이 0에 수렴한다
```

> **경로 2가 성립하는지는 그 메모리의 외부 대역폭에 달려 있다.** 전송이 GPU 연산 시간보다 짧으면 숨고, 길면 전송이 지배한다. **이 경계가 "재활성이 잦은 세션을 어느 계층에 둘 수 있는가"를 정하며**, 결정점 B의 후보 형성에 직접 들어간다. 각 메모리가 어느 쪽인지는 시뮬레이션에서 판정한다.

### 4.6 이 구조가 무엇을 얻는가 — 자원 병렬화

**Attention 오프로드의 이득은 Decode step이 빨라지는 데 있지 않다.** 오프로드된 step은 근접 연산 유닛의 성능에 따라 GPU보다 느릴 수 있다. 이득은 **GPU와 연산형 메모리가 직렬이 아니라 병렬로 도는 것**에서 나온다.

```text
Agent 한 턴 = Incremental Prefill 1회 + Decode N회

GPU만 사용                          Attention 오프로드
─────────                          ──────────────────
GPU: [prefill][dec][dec]...[dec]   GPU : [prefill][w][w]...[w]
      연산한계  대역폭한계 (직렬)    노드:         [attn][attn]...[attn]
                                            └─ 두 자원이 병렬 ─┘
턴 시간 = prefill + N×(attn+weight)  턴 시간 = max(GPU 점유, 메모리 점유)
```

**Prefill은 연산 한계라 GPU가 잘하고, Decode Attention은 대역폭 한계라 메모리가 잘한다.** 서로 다른 자원을 쓰므로 겹칠 수 있다 — 이것이 이기종 메모리에 연산 기능을 넣는 이유다. 여기에 **상위 계층 용량이 비어 동시 수용 세션이 늘어나는 효과**가 더해진다.

> **이 두 이득은 Latency 지표에 나타나지 않는다.** 그래서 §11의 주 지표가 처리량(M-P1)이고 TPOT는 보조다. **Cost Model이 GPU 점유와 메모리 점유를 별도 자원으로 두고 `max`로 합치지 않으면, 이 DP의 논거 자체가 측정되지 않는다.** 실제로 얼마나 얻는지는 시뮬레이션이 답한다.
...
#### 결정점 A용 특성 — 이번 턴의 예상 Decode 길이

위 넷은 **결정점 B**(유휴 중 어디에 둘 것인가)의 입력이다. **결정점 A**(이번 턴의 Decode를 어디서 수행할 것인가)는 다른 것을 묻는다.

| 특성 | 정의 | 무엇을 결정하는가 | 결정 시점에 |
|---|---|---|---|
| **Expected Decode Length** | 이번 턴에 생성할 토큰 수 | §4.6의 자원 병렬화가 몇 step 동안 이어지는가. 짧으면 옮기는 비용을 회수 못 한다 | **추정 필요** |
| **세션 KV 크기** | 현재 누적 History 크기 | 옮기는 비용과 상위 계층 점유량 | **선언 가능** |
| **재활성 연산 (Next Operation)** | 이번 턴 Decode가 요구하는 Attention 원시 연산 집합 | §4.4의 오프로드 가능 여부 | **선언 가능** |

**Expected Decode Length도 Agent Tool Info가 근거가 된다.** 직전 턴이 Tool Call을 낸 것이면 이번 턴의 출력은 대개 짧은 구조화 텍스트이고, 최종 답변 단계면 길다. §5.4의 타당성 검토가 이 특성에도 그대로 적용된다 — 근거 강도는 Next-access Time과 비슷한 "중간"이며 분산이 크다.
...
| **Expected Decode Length** (결정점 A) | **중간** | 턴 성격(Tool Call 턴 vs 최종 답변 턴)이 출력 길이와 상관이 있고 로그에서 측정된다 | Next-access Time과 같은 문제 — **분산이 크다.** 최종 답변 턴의 길이는 사실상 출력 길이 예측이며 어렵다 |
...
§4.4의 손익분기 조건은 **연산형 메모리가 내부 대역폭을 온전히 쓴다는 가정** 위에 있다. 실제 PNM에서 Attention 커널이 1.1 TB/s를 얼마나 달성하는지는 구현에 달려 있으며 공개 수치가 없다. 달성률이 50%면 오프로드 쪽 Step 비용이 두 배가 되어 손익분기가 크게 내려간다.
...
| Class | 조건 | 배치 후보 | 근거 |
|---|---|---|---|
| **Hot & Compute-heavy** | Next-access Time 짧음 + Reuse Probability 높음 + 재활성 연산이 오프로드 가능 | HBM, **Custom HBM 노드** | 곧 쓰이고 확실히 쓰인다. §4.4대로 Custom HBM 노드에 두고 오프로드하면 로컬 HBM과 대등한 속도로 HBM 용량을 아낀다 |
...
4. **§4.6의 자원 병렬화가 나타나는 유일한 지표다.** Attention 오프로드는 Step latency 기준으로는 손해로 보일 수 있다. 이득은 **GPU와 연산형 메모리가 병렬로 도는 것**과 **용량 확대로 배치가 커지는 것**에서 나오며, 이 둘은 처리량으로만 환산된다.

> **따라서 Cost Model은 GPU 점유 시간과 메모리 점유 시간을 별도 자원으로 모델링해야 한다.** 둘을 하나의 직렬 시간으로 합산하면 §4.5의 병렬화가 사라지고 이 DP의 논거가 측정되지 않는다. 최소한 다음 두 자원을 구분한다.
>
> ```text
> GPU 자원   : Prefill 연산 + Projection/FFN + (오프로드 안 한 경우) Decode Attention
> 메모리 자원 : (오프로드한 경우) Decode Attention
> 턴 시간     = max(GPU 자원 점유, 메모리 자원 점유)      ← 합이 아니라 max
> ```
...
> **주 지표가 아니다.** Attention 오프로드는 TPOT를 악화시킬 수 있지만 §4.6의 자원 병렬화로 처리량은 개선될 수 있다. **TPOT만 보고 판단하면 이 DP의 결론이 정반대로 나올 수 있다.** TPOT는 SLO 제약(M-P1)을 만족하는지 확인하고 오프로드가 대화형 예산을 넘지 않는지 보는 용도로 쓴다.
...
> **이 지표가 재지 않는 것:** GPU Occupancy와 에너지 항이 없다. Mode C에서 Attention을 메모리로 보내면 GPU 연산 유닛이 비어 다른 일을 할 수 있는데 그 이득이 보이지 않으므로, **연산 가능 메모리에 관한 모든 수치는 이득의 하한**이다.
>
> **Mode A와 Mode C를 같은 메모리에 대해 각각 보고한다.** §3.3대로 Custom HBM 노드는 외부/내부 비대칭이 크므로 같은 메모리에서 두 Mode의 비용이 크게 갈린다 — 그 격차의 크기는 시뮬레이션이 답한다. 이 차이가 이 DP에서 Attention 오프로드가 만드는 이득의 실체이고, 메모리 이름만으로는 드러나지 않는다.
...
**메모리별로 나눠 보고한다** — §3.3/§4.4에서 보였듯 두 연산형 메모리의 이득 구조가 다르다. **Custom HBM 노드는 오프로드하면 로컬 HBM과 대등한 속도로 5배 용량을 쓰는 것**이고, **CXL-PNM은 오프로드해도 로컬 HBM의 7.3배 느리되 용량당 비용이 싼 것**이다. 합산하면 이 차이가 사라진다.
...
#### M-P7b. GPU 점유율 / 자원 병렬도

§4.5의 이득이 실제로 발생했는지를 직접 관측한다. M-P1이 "처리량이 올랐다"를 말한다면 이 지표는 **"왜 올랐는가"** 를 말한다.

```text
GPU Attention 점유 시간 비율 = Decode Attention에 쓴 GPU 시간 / 전체 GPU 시간
                              ← 오프로드가 줄이려는 대상

자원 병렬도 = (GPU 점유 + 메모리 점유) / max(GPU 점유, 메모리 점유)
             1.0 = 직렬 (겹치지 않음)   2.0 = 완전 병렬

병목 자원 비율 = GPU가 병목인 시간 / 메모리가 병목인 시간
```

**병목이 어디인지를 함께 보고한다.** §4.2의 균형점을 넘으면 병목이 GPU로 넘어가고, 그 지점부터는 메모리를 개선해도 처리량이 오르지 않는다. **이 전환점이 관측되지 않으면 어느 자원에 투자해야 하는지 답할 수 없다.**

> 이 지표는 §14의 "GPU Occupancy 실측"과 짝이다. 시뮬레이션에서는 자원별 점유 시간으로 대리 측정하고, 실측에서는 GPU 커널 점유율로 확인한다.
...
| **턴당 생성 길이** | §4.4의 손익분기가 이 값에 걸려 있다 |
| **유휴 시간 분포 (Tool 실행 시간)** | Mode A/B/C 선택을 직접 좌우 |
| **배치 크기** | §4.3의 계층당 왕복 비용이 배치 크기에 반비례 |
| **연산형 메모리의 Attention 연산 성능 (TFLOPS)** | **가장 중요한 축.** §4.2의 균형점을 걸쳐서 훑는다 — 그 아래에서는 연산 한계, 위에서는 대역폭 한계가 된다 |
| **Custom HBM 노드 내부 대역폭** | §3.5의 가정값(8.0 TB/s). §4.2의 균형점이 이 값에 비례한다 |
| **턴당 생성 토큰 수 N** | §4.6의 처리량 이득과 병목 자원이 N에 따라 바뀐다. §5.1의 Expected Decode Length가 이 값을 추정한다 |
| **Scale-up fabric 세대 (NVLink 5 / 차세대 / UALink 고밀도)** | Custom HBM 노드의 외부 대역폭. 비대칭이 줄면 Mode A/C 격차와 §4.5 스트리밍 성립 여부가 함께 바뀐다 |
...
- §3.5의 가정값(Custom HBM 노드의 용량·내부 대역폭·fabric 지연, HBF 쓰기 특성)이 결론에 영향을 줄 수 있으므로, 선정 문서는 **그 값이 어느 범위일 때 결론이 유지되는지**를 함께 밝혀야 한다.
...
| **GPU Occupancy 실측 (M-P7b)** | §4.6의 자원 병렬화가 이 DP의 핵심 논거다. 시뮬레이션은 자원별 점유 시간으로 대리 측정하지만, 실제 커널 스케줄링에서 GPU와 메모리가 정말 겹치는지는 실측해야 한다 |
| **연산형 메모리의 유효 TFLOPS (§3.6 우선순위 1)** | §4.6대로 결론을 지배한다. HW팀 확인 항목 1·3번 |
| **계층당 왕복 지연 실측 (§4.3)** | 경로 지연은 가정값이며, `계층 수 × 지연`이 오프로드 가능 여부를 가른다 |
| **Custom HBM 노드 내부 대역폭·용량·fabric 지연 (§3.5 가정값)** | 전부 가정값이고, §4.4의 핵심 결론("오프로드 시 로컬 HBM과 대등")을 직접 만든다. 실제 노드 스펙 없이는 이 결론을 인용하면 안 된다 |
